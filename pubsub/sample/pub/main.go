...
import (
	...
	zpub "github.com/elvisNg/broccoliv2/pubsub/pub"
...
func pubManager() {
	...
	for {
		...
		// msg.Body = body
		// log.Printf("Publish Sample message: %+v\n", msg)
		// err := m.Publish(context.Background(), "broccoli", h, msg)
		// if err != nil {
		// 	fmt.Println("err==>>", err)
		// }
		...
		sreq := &brokerpb.RequestSample{Message: "hello world!", Count: 100, Finished: true}
		log.Printf("Publish SampleRequest message: %+v\n", sreq)
		err = m.Publish(context.Background(), "broccoli", &brokerpb.Header{Category: "samplerequest", Source: "broccoli"}, sreq)
		if err != nil {
			fmt.Println("err==>>", err)
		}
		...
		// }
		// log.Printf("Publish pbstruct message: %+v\n", sMsg)
		// err = m.Publish(context.Background(), "broccoli", &brokerpb.Header{Id: fmt.Sprint(time.Now().Unix()), Category: "pbstruct", Source: "broccoli"}, sMsg)
		// if err != nil {
		// 	fmt.Println("err==>>", err)
		// }
		...
		req := &Request{Message: "hello world!", Count: 100, Finished: true}
		log.Printf("Publish JSON Request message: %+v\n", req)
		err = m.Publish(metadata.NewContext(context.Background(), metadata.Metadata{"for-handler": "json-request"}), "dzqz", &brokerpb.Header{Id: fmt.Sprint(time.Now().Unix()), Category: "jsonrequest", Source: "broccoli"}, req)
		if err != nil {
			fmt.Println("err==>>", err)
		}
